...
# def _process_data(data, vocab, pos_tags, chunk_tags, maxlen=78, onehot=False):
    if maxlen is None:
    ...
    x = pad_sequences(x, maxlen, padding='post')  # right padding

    # lef padded with -1. Indeed, any integer works as it will be masked
    # y_pos = pad_sequences(y_pos, maxlen, value=-1)
    # y_chunk = pad_sequences(y_chunk, maxlen, value=-1)
    y_pos = pad_sequences(y_pos, maxlen, value=0, padding='post')
    y_chunk = pad_sequences(y_chunk, maxlen, value=0, padding='post')
