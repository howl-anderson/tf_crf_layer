...
def test_masking_fixed_length():
    ...
    model.summary()
    model.fit(x, y, epochs=1, batch_size=1)  # TODO: why batch_size can not be other number than 1
    ...
    # assert (y_pred[1, :5] == 0).all()  # left padding not working currently due to the tf.contrib.crf.*
